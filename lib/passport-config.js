...
module.exports = function(passport) {
  ...
  const callbackURL = (process.env.NODE_ENV == 'production')?
    'https://salty-fortress-40948.herokuapp.com/auth/facebook/callback':
    'http://localhost:3000/auth/facebook/callback';
  passport.use(new FacebookStrategy({
    // 이 부분을 여러분 Facebook App의 정보로 수정해야 합니다.
    clientID : process.env.FBID || '1702765906464886',
    clientSecret : process.env.FB_SECRET || '2a21575d4cc0b4c8f15d17c5e3da3506',
  ...
  passport.use('kakao-login', new KakaoStrategy({
    clientID:'5c23d9458354352fae30d22edaa7763f',
    clientSecret:'9SEZKIeaY2kX5IqMrk42tFMZxMfdYIzY',
    callbackURL: 'http://localhost:3000/auth/kakao/callback'
  },
  async function(accessToken, refreshToken, profile, done) {
    console.log('kakao-login', profile);
    user.kakao.id = profile.id;
    user.kakao.username=profile.username;
    user.kakao.displayName=profile.displayName;
